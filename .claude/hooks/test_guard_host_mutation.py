...
CASES = [
    ...
    ("deny", "gcloud compute instances create rescue --metadata=startup-script='echo hi'"),
    ("deny", "gcloud compute project-info add-metadata --metadata-from-file=shutdown-script=x.sh"),
    ("deny", "NIXOS_NO_CHECK=1 true"),
    ("ask", "gcloud compute instances remove-metadata nagare-01 --keys=startup-script,ssh-keys,enable-oslogin"),
