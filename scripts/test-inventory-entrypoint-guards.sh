...
refuse_image_free_deploy() {
  ...
  if ! grep -q 'requires --image-resource' "$fixture_root/out"; then
    printf '%s did not enter reviewed planning:\n' "$label" >&2
...
refuse_unreviewed_change 'unreviewed site rollback' site rollback v1 -f "$fixture_root/missing-site.hs"
refuse_unreviewed_change 'unreviewed preview delete' site preview delete p1 -f "$fixture_root/missing-site.hs"
refuse_unreviewed_change 'unreviewed app delete' app delete app1
