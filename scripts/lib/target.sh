...
_NAGARE_CONTEXT_VARS=(
  ...
  NAGARE_IMAGE_BUCKET NAGARE_BACKUP_BUCKET NAGARE_NIX_CACHE_ENABLED
  NAGARE_NIX_CACHE_BUCKET NAGARE_BASE_DOMAIN NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED
...
_nagare_resolve_context() {
  ...
  export NAGARE_NIX_CACHE_ENABLED="${NAGARE_NIX_CACHE_ENABLED:-0}"
  export NAGARE_NIX_CACHE_BUCKET="${NAGARE_NIX_CACHE_BUCKET:-${CLOUDSDK_CORE_PROJECT}-nagare-nix-cache}"
  export NAGARE_BASE_DOMAIN="${NAGARE_BASE_DOMAIN:-apps.example.com}"
  export NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED="${NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED:-0}"
  ...
  case "${NAGARE_NIX_CACHE_ENABLED}" in
    0|1) ;;
    *)
      echo "nagare: NAGARE_NIX_CACHE_ENABLED must be 0 or 1" >&2
      return 1
      ;;
  esac
  if [ "${NAGARE_MODE}" = "local" ] && [ "${NAGARE_NIX_CACHE_ENABLED}" = "1" ]; then
  ...
  case "${NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED}" in
    0|1) ;;
    *)
      echo "nagare: NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED must be 0 or 1" >&2
      return 1
      ;;
  esac
  if [ "${NAGARE_MODE}" = "local" ] && [ "${NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED}" = "1" ]; then
    echo "nagare: NAGARE_EXTERNAL_DOMAIN_TLS_ENABLED=1 is cloud-only" >&2
    return 1
  fi
...
NAGARE_BUILDER_PROJECT="${NAGARE_BUILDER_PROJECT:-${TARGET_PROJECT}}"
NAGARE_BUILDER_ZONE="${NAGARE_BUILDER_ZONE:-${TARGET_ZONE}}"
NAGARE_BUILDER_INSTANCE="${NAGARE_BUILDER_INSTANCE:-nix-builder-x86}"
export NAGARE_BUILDER_PROJECT NAGARE_BUILDER_ZONE NAGARE_BUILDER_INSTANCE
