...
- EP-87 and EP-88 are complete. The Haskell side now has a user-level context store, active-context
  resolution, `nagarectl context` management commands, a global `--context` selector, and named
  `nagarectl init NAME` writes. The remaining initiative work is the shell/direnv resolver and guardrail
  (EP-89), per-context Pulumi state/projection (EP-90), applied manifest/Nix de-hardcoding (EP-91), and
  docs/migration (EP-92).
