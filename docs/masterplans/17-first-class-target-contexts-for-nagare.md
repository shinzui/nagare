...
| 88 | nagarectl context command group and --context selection | docs/plans/88-nagarectl-context-command-group-and-context-selection.md | EP-87 | None | Complete |
| 89 | Shell and direnv context resolution and the context-aware guardrail | docs/plans/89-shell-and-direnv-context-resolution-and-the-context-aware-guardrail.md | EP-87 | None | Complete |
...
- [x] EP-88: `nagarectl context list|current|use|show|create|delete` implemented against the EP-87 store; global `--context` flag wired into every command's target resolution.
- [x] EP-88: `nagarectl init` writes a named context (not a bare `nagare.target.env`); `nagarectl --context <name>` targets that context with no `cd` and no file edit.
- [x] EP-89: `scripts/lib/target.sh` and `.envrc` resolve the active context from the EP-87 store with the same precedence; `NAGARE_CONTEXT=<name> just <recipe>` targets that context; justfile/scripts unchanged in how they read the environment.
- [x] EP-89: `_require_target_project` asserts against the active context's project in cloud mode and steps aside in `mode=local`, replacing the `NAGARE_MODE=local` special-case.
...
- **Back-compat correction: the pure-default case must keep exporting the `tan-nb-exp` defaults (EP-89
  ...
  `us-west1-a` defaults exactly as today. EP-89 implements this correction.
...
- **EP-88 confirmed explicit target threading has to cross executable/library boundaries.** Several
  target-consuming behaviors were hidden behind library entry points (`runAppDeploy`, `runWorkerDeploy`,
  `runDbCreate`, and Docker auth in static/server deploys), not only direct calls in `Main.hs`. EP-88 now
  resolves the selected `TargetProfile` once in the executable and passes it down, leaving
  `resolveTargetProfile` as a compatibility wrapper for older call sites. Date: 2026-07-01.

- **EP-89 reconciled the shell default export with EP-87/Haskell resolution.** The EP-89 draft still
  contained an older idea that bash should leave `CLOUDSDK_CORE_PROJECT` unset in the built-in default
  branch. Implementation follows the MasterPlan correction and EP-87 behavior instead: the shell exports
  the historic `tan-nb-exp`/`us-west1` defaults, so unqualified `gcloud` and scripts see the same target
  bundle as `nagarectl`. Date: 2026-07-01.
...
- EP-87, EP-88, and EP-89 are complete. The Haskell and bash/direnv sides now agree on the context
  store, selection precedence, and exported target contract; `nagarectl context` manages the store, and
  scripts/just recipes pick up the active context through `scripts/lib/target.sh`. The remaining
  initiative work is per-context Pulumi state/projection (EP-90), applied manifest/Nix de-hardcoding
  (EP-91), and docs/migration (EP-92).
