...
- [x] EP-45 (2026-06-10): `nagarectl db create ENGINE NAME` generates the credential Secret (IP3) and provisions the resources in order (Secret, then PVC/ConfigMap/Service/StatefulSet, then wait for rollout); idempotent password; demonstrated via the real CLI `--dry-run` for all three engines. **Live leg VERIFIED 2026-06-10** against `nagare-01`: `db create` for all three engines provisioned and rolled out Ready — `auditpg` (**PostgreSQL 18.4**, write/read roundtrip OK), `auditcache` (**Redis 8**, `PING`→PONG, SET/GET OK), `auditolap` (**ClickHouse 25.8.24.21**, `SELECT version()` OK); `db list`/`db get` rendered the live inventory and secret key names.
...
- [x] EP-46 (2026-06-10): An app declares `databases`; `nagarectl deploy` injects the per-engine connection env — host/port/user/db literals + `*_PASSWORD`/`*_URL` Secret refs to `nagare-db-<name>` — merged via `mergeGenerated` (generated wins over user env); collision + missing-db errors clear; verified via render-demonstration + unit tests (195 nagarectl tests). **Live VERIFIED 2026-06-10**: `postgres-app`'s `deploy --dry-run` against the running cluster read the real `nagare-db-pg-main` Secret and injected `POSTGRES_HOST`/`POSTGRES_DB`/`POSTGRES_PORT` literals plus `DATABASE_URL`/`POSTGRES_PASSWORD` `secretKeyRef`s to `nagare-db-pg-main`. Server sites deferred (no `ServerSite.databases` field in v1).
- [x] EP-47 (2026-06-10): `nagarectl db backup NAME` renders+applies a two-container dump Job to the GCS layout `databases/<name>/<ts>.<ext>`; keep-last-N retention reuses `snapshotsToPrune`; daily self-pruning CronJob renders and is provisioned at `db create` (unless `retention = Delete`). **Live upload VERIFIED 2026-06-10** — `db backup auditpg` ran the dump Job and wrote `gs://tan-nb-exp-nagare-backups/databases/auditpg/20260610T230713Z.sql.gz` (confirmed via `gsutil ls`). _The EP-43 flannel/metadata routing regression that previously blocked this no longer reproduces on the current cluster (see Decision Log / Surprises)._
- [x] EP-47 (2026-06-10): `nagarectl db restore NAME BACKUP_ID` renders a scratch-first restore Job (`--into-live` opt-in); restore drill documented; disaster-recovery runbook + user backup guide updated (incl. the routing caveat). 207 nagarectl tests pass. **Restore drill VERIFIED live 2026-06-10**: restoring `20260610T230713Z` into the scratch target `auditpg_restore_scratch` recreated the `audit_probe` table with `COPY 1` (the exact row written before the backup), proving the round-trip and the scratch-first/promote-manually flow.
- [x] EP-48 (2026-06-10): User guide `docs/user/managed-databases.md` written + cross-linked; one end-to-end example per engine (`postgres-app`, `redis-cache`, `clickhouse-analytics`) renders via `db create --dry-run` (configs emit valid JSON; ClickHouse shows its memory limit + dual ports). **Live deploy + restore drill EXECUTED and VERIFIED 2026-06-10** (see EP-45/46/47 entries above): all three engines created and queried live, connection-env injection proven against a live Secret, a real GCS backup written, and a scratch restore round-tripped the data. The flannel/metadata routing regression no longer reproduces.
...
- Decision (2026-06-10): Mark the deferred live legs of EP-45/46/47/48 **VERIFIED** after executing them
  end-to-end against the running `nagare-01` cluster (it was found still up). Evidence: `db create` for
  Postgres 18.4, Redis 8, and ClickHouse 25.8.24.21 all rolled out Ready and answered live queries;
  `postgres-app deploy --dry-run` injected connection env from the live `nagare-db-pg-main` Secret;
  `db backup auditpg` wrote a real object to `gs://tan-nb-exp-nagare-backups/databases/auditpg/…`; and a
  scratch restore round-tripped a known row (`COPY 1` into `auditpg_restore_scratch`).
  Rationale: the only remaining gap in this MasterPlan was the powered-off-VM live leg; with the VM up,
  the deferred verification is now real rather than render-only.
- Decision (2026-06-10): **The EP-43 flannel/metadata routing regression no longer reproduces** — EP-47's
  live GCS backup, the leg that regression specifically blocked, succeeded on the current cluster. The
  caveat is retired; if it recurs, re-open against the cluster networking, not the backup renderer.
