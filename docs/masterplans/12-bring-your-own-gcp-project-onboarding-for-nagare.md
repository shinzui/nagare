...
|---|-------|------|-----------|-----------|--------|
| 60 | Target profile model and configurable isolation guardrail | docs/plans/60-target-profile-model-and-configurable-isolation-guardrail.md | None | None | In Progress |
...
- [ ] EP-60: `nagare.target.env.example` + git-ignore entry exist; `scripts/lib/target.sh` loads the profile and runs the configurable fail-closed preflight; `.envrc` sources the profile with `tan-nb-exp` fallbacks; `CLAUDE.md` isolation section rewritten to the configurable model.
