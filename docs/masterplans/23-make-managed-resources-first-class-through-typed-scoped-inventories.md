...
2026-09-24: EP-148 extended saved-review replay coverage to a 12-member application with Service, route, three workers, database and backup, scheduled task, and release metadata. A recording adapter verifies exact native membership and dependency order. The test exposed a missing DomainMapping hostname alias during review reconstruction; the shared native binder now derives that claim from the DomainMapping address. Broader live-provider, broker-topic, preview, CDN, and hook proof remains open, so M1 and the EP-150 gate are unchanged.

2026-09-24: EP-148 added a single-invocation reviewed live app route when an accepted image resource and explicit tag are supplied. The shared command service publishes and reloads the immutable review before applying through the existing journal; an isolated local-store/recording-adapter test proves that path for one ConfigMap. Adoption and other lifecycle decisions still require separate review. Legacy no-image deployment, build publication, hooks, CDN, and native app command proof remain open; M1 and EP-150 status are unchanged.
