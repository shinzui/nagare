...
A second disposable ConfigMap probe confirmed that applying the same value after a create adds Apply co-ownership without releasing the create request's Update ownership; the next changed server-side apply still conflicts. The initial transport therefore keeps create-only and UID/resourceVersion-bound non-forced apply as separate guarded operations, and may refuse a legitimate update until a per-kind ownership transition is proven. It does not use forced apply to bypass an unreviewed owner.
