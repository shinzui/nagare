...
| Shared cluster Namespaces and personal Job quota | platform foundation | `Nagare.Inventory.Components.Foundation.compileFoundation` | Kubernetes adapter; canonical native objects | `InventoryFoundationSpec` checks six claims, quota ordering, composition, and native binding | Namespace runtime and quota admission remain Kubernetes-owned | `cluster-bootstrap`, `local-bootstrap`, and `job-runs-bootstrap` still call kubectl directly | adapter-ready |
