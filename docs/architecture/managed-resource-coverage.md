...
| Cache database, Attic workload, named cache, and client public key | platform cache | `Nagare.Inventory.Cache.compileCacheComponent` composes 16 typed resources | Kubernetes adapter and `scripts/inventory-cache-transport.sh` for the named Attic cache | `InventoryCacheSpec` composes the complete scope and checks the subprocess boundary; disposable database transaction exercises Kubernetes resume | PostgreSQL data and Attic signing identity are retained; client key is a typed generated output | Generated native members still need the bootstrap review entry point; `cluster/bootstrap/nix-cache/install.sh` remains the live entry point | adapter-ready |
| Shared cluster Namespaces and personal Job quota | platform foundation | `Nagare.Inventory.Components.Foundation.compileFoundation` | Kubernetes adapter; canonical native objects | `InventoryFoundationSpec` checks six claims, quota ordering, composition, and native binding; disposable cluster verifies guarded Namespace label update | Namespace runtime and quota admission remain Kubernetes-owned | `cluster-bootstrap`, `local-bootstrap`, and `job-runs-bootstrap` still call kubectl directly | adapter-ready |
