...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Pulumi cloud stack preview/apply, including APIs, IAM, network, disks, buckets, DNS/CDN, registry, VM, and cache resources | platform cloud foundation | `Nagare.Inventory.Cloud.compileCloudScope`; `infra/pulumi/src/resourceDeclarations.ts` consumes the canonical registration bundle | `Nagare.Inventory.Adapters.Pulumi`; exact Pulumi saved plan | `InventoryCloudSpec`; `infra/pulumi/test/resourceInventory.test.ts`; existing reviewed-plan guard fixtures | Pulumi provider bookkeeping is explicitly classified in the native registration bundle | `nagarectl infra preview/apply` still uses the pre-inventory reviewed-plan service; production registry construction remains to migrate | adapter-ready |
| Guarded NixOS activation | platform host | `Nagare.Inventory.Host.compileHostScope` | `Nagare.Inventory.Adapters.Host`; `scripts/host-switch.sh`; `nixos/lib/nagare-safe-switch-client.sh` | `InventoryHostSpec`; `nix/checks/scripts/test-host-switch-identity.sh`; host auto-rollback VM check | Nix derivations/files remain internal to the host executor; the resource is the evaluated system and declared durable/configuration inputs | Transport now rejects a foreign inventory child and emits a fresh-login commit receipt; `just host-switch` remains a legacy public entry point | adapter-ready |
...
| NixOS image object and GCE image publication | platform artifacts | `Nagare.Inventory.Artifact.compileArtifactScope` | `Nagare.Inventory.Adapters.Artifact`; `scripts/upload-images.sh` | `InventoryArtifactSpec`; `scripts/test-upload-images.sh`; bucket ownership guard | Nix build and builder-side upload are bounded transport; Pulumi owns the destination bucket | Publisher cannot create the bucket, and an inventory child cannot rewrite Pulumi config; direct invocation still performs the legacy config write | adapter-ready |
...
| Attic/Nix cache OCI image publication | platform artifacts | Artifact resource bundle | Artifact adapter; `cluster/bootstrap/nix-cache/publish-image.sh` | `InventoryArtifactSpec`; `scripts/test-inventory-transport-guards.sh`; immutable pin/digest checks in the publisher | Registry authentication and copy are bounded transport | Transport emits an inventory receipt and rejects a foreign child; cluster bootstrap still invokes it directly | adapter-ready |
