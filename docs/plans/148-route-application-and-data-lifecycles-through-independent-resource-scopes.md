...
- [ ] M2: Route CDN/DNS through explicit provider ownership. Cloudflare application records need their own claimed addresses; the Google backend and its DNS zone remain platform/Pulumi owned and require an owner-composed contribution or exact reference.
...
M1 complete for the reviewed Kubernetes application and standalone lifecycle. Stable identities, accepted data and broker bindings, auth contributions, release history, environment and Secret references, and per-tag hook scopes are compiled into reviewed declarations; unsupported CDN/build intent refuses. The 799-test CLI suite, DSL suite, and disposable two-worker native review/resume passed. A bounded reviewed topic retention update is implemented. The overall plan remains in progress: M2 CDN/publication and other topic updates, M3 data and operational actions, and M4 direct-path removal and full scope-isolation proof remain open.
