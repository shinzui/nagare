...
- [x] (2026-09-24) M2 partial: Exact Runtime `env sync --save-plan` and `secret sync --version --save-plan` compile separate ConfigMap and Secret scopes and save shared inventory reviews. Private native bytes match the existing envFrom targets; later app scope replacements leave these channels' revisions and values alone. Build, Preview, and merge intent channels remain open.
