...
- [x] (2026-09-24) M1/M2 partial: Reviewed application planning binds topic-free app-level broker references to an accepted standalone broker Service and StatefulSet in the same cluster/namespace. Kafka connection env derives from that accepted identity, and workload declarations/private native members both depend on the Service. Unaccepted, duplicate, and topic-bearing bindings refuse; per-workload broker bindings and logical topic operations remain open.
...
- [x] (2026-09-24) M2 partial: `env sync --reconcile-exact --save-plan` compiles the typed Runtime ConfigMap into its own application environment scope and saves a shared inventory review. The private native bytes match the existing envFrom target; later app scope replacements leave this channel's revision and values alone. Build, Preview, Secret, and merge intent channels remain open.
