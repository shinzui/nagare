...
provenance:
  ...
  revisions:
    ...
    - model: "gpt-6-astra"
      harness: "codex-cli"
      at: 2026-09-26T05:46:12Z
      mode: "implement"
      note: "Bind Preview-scoped server environment and Secret references to reviewed previews"
    - model: "gpt-6-sol"
      harness: "codex-cli"
      at: 2026-09-26T14:21:47Z
      mode: "implement"
      note: "Add reviewed manual database backup checksum receipts"
    - model: "gpt-6-astra"
      harness: "codex-cli"
      at: 2026-09-26T18:30:21Z
      mode: "implement"
      note: "Route signed static webhooks through reviewed site submission and remove duplicate site deploy effects"
    - model: "gpt-6-astra"
      harness: "codex-cli"
...
Newly compiled reviewed database scopes now emit their scheduled backup CronJob without the legacy inline keep-last-N object-store deletion. The supported inventory compiler is the only exported native database binder; its saved private CronJob bytes upload, read back, and SHA-256-check the exact object before the Job succeeds. `db disable-backup-prune --save-plan` can replace an earlier accepted CronJob through a match against the known older renderers while preserving its sibling declarations and native evidence. Existing CronJobs retain their earlier script until that review is applied. Legacy direct database commands retain their earlier renderer for unadmitted contexts. The scheduled readback is an execution check without a durable per-object receipt or expiry policy; scheduled pruning remains M3 work.

An accepted database can save a manual backup review with `db backup NAME --backup-id ID --save-plan DIR`. The fixed Job has its own stable scope and namespace-isolated object key under `manual-databases/<namespace>/<name>/` outside the legacy schedule pruning prefix, an explicit retain or UTC expiry choice, the accepted source scope revision, StatefulSet/PVC UIDs, and the exact stored-byte SHA-256 check. A `SnapshotData` operation journals completion after the Job. Execution reloads accepted source native bytes and checks both physical UIDs before submit and verification; a changed accepted backup intent under the same ID refuses. The Job renders create-only uploads for both the backup and a `.receipt.json` object with the checked SHA-256, source proof, object URL, and expiry; it reads both objects back before success. The inventory operation verifies the completed upload Pod's receipt. Reviewed PostgreSQL scratch restore consumes that receipt and rechecks current object bytes. Expired finite-expiry manual backups have a separate reviewed `PruneData` Job that checks exact bytes and provider versions, blocks accepted restore dependencies, and has no automatic retry. Scheduled pruning, live-target and other-engine restore, and live object-store provider proof remain M3 work. A partial upload or deletion requires explicit recovery. Source checks are not atomic with Job execution. Direct manual backup remains guarded after inventory initialization.

A scope replacement can now explicitly retain selected removed members while keeping the scope and siblings accepted. `inventory plan --retain-resource` publishes that decision against the accepted revision and live physical identity; admission checks complete removed-member coverage and reserves the old claim. The focused memory-store transaction and all 830 CLI tests passed; the executable build, Haskell style check, strict user-documentation validation, and diff check also passed. Retention alone does not stop a live CronJob.

`task delete --save-plan` now selects three successive saved reviews for an accepted task: suspend the exact CronJob from accepted private native bytes, retain that suspended member while preserving sibling declarations, then collect its exact retained incarnation. Each stage must be applied before planning the next. Direct `--yes` remains guarded in initialized contexts, and legacy run-history ConfigMaps are outside this CronJob-only sequence. The initial 831-test CLI suite, executable build, Haskell style, strict user-documentation validation, and isolated direct-delete guard passed. The complete CLI/provider replay has since passed as described below; the remaining M3 operations are still open.

A disposable Kubernetes review transaction exercises all three task-deletion stages against a live CronJob and sibling ConfigMap. It exposed native evidence reconstruction that dropped an unchanged sibling after a scope revision; lookup now rebinds digest-identified native bytes from an earlier immutable review to the current accepted declaration. The offline carry-forward regression also covers source-only revisions. A second disposable transaction now runs the actual `task delete --save-plan` and `inventory apply --yes` command pair for suspension, retention, and collection. It keeps the sibling's physical UID fixed and revealed two CLI collection adapter errors: planning reopened source paths despite accepted native bytes, and apply tried to reconstruct a collected member from desired scopes. Both paths now use its immutable retained history. All 834 CLI tests, the executable build, Haskell style, strict user-documentation validation, and diff check pass. The disposable cluster was stopped afterward. The rest of M3 remains open.

Preview-scoped inline server environment now enters only the preview Service, and its Secret references require exact accepted Secret dependencies. The local 459-test DSL and 834-test CLI suites and strict user-documentation validation pass. The repository-wide Fourmolu check still reports broad formatting differences outside this change; the Haskell structure check passes. M2 remains open for Build Secret publication and combined provider acceptance.

The generic CLI now constructs planning and execution adapters only for selected Pulumi, host, artifact, broker, CDN, and native resources. A local isolated script first applies an empty application scope beside accepted Pulumi and host owners, then plans a selected Namespace contribution with only `kubectl` invoked. The host owner carries a valid activation operation but has no local host configuration; before the execution change, applying the empty review refused while trying to load that unrelated host. The script also invokes the actual `app deploy --save-plan` with a prepublished image and accepted Namespace beside those unrelated owners; only Kubernetes observations run and the review contains Kubernetes operations. A recording `kubectl` then applies that saved app review, creates exactly the Service and release-history ConfigMap, and leaves the unrelated accepted revisions fixed without invoking another provider executable. Its optional live mode applied a worker-only app on disposable local k3d with a preloaded HTTP image: the real Kubernetes provider created one available Deployment and its release-history ConfigMap, while unrelated Pulumi, host, artifact, and foundation members remained untouched. The 835-test CLI suite, executable build, Haskell structure check, and diff check passed before the recording apply extension; both recording and live probes passed afterward. This proves one real app-command provider path and local credential isolation. Native Knative Service application apply, full combined M2 provider membership, and remaining direct-path removal stay open.

Reviewed application deploy also declares the per-Service release-history ConfigMap, carries forward accepted entries, and offers exact adoption of existing direct-deploy history. Supported static and server production site scopes now declare their Service, domains, release history, server PVCs, accepted runtime Secret dependencies, and supplied TLS Secret dependencies, with exact import of existing direct objects. Site rollback now selects a release from accepted private history and reviews the prior image publication without adding a history entry. Static and server preview deployment has a separate reviewed scope with four accepted overlay stores, exact adoption of existing direct static previews, and reviewed retirement followed by collection. Server previews bind distinct PVCs with explicit recovery for retained volumes and accepted Preview-scoped inline Secret references; Preview-only entries stay out of production Services. Delete-policy preview PVCs can be conditionally collected after their Service, while durable PVCs remain retained. Manual runs of accepted CronJobs now compile stable Jobs in independent scopes from exact saved native evidence; Job collection uses conditional deletion and background pod cleanup. Reviewed application hooks have per-tag Jobs with explicit affected-resource proofs. Server Build Secret publication and other operational commands remain open.
...
In initialized inventory contexts, live `task run` requires the stable-ID reviewed route, even for a newly named CronJob. Direct `task delete --yes` refuses there; `task delete --save-plan` saves the next suspension, retention, or collection review for an accepted schedule. The read-only run/delete previews remain available; uninitialized contexts retain the legacy commands.
...
App stop and restart require an accepted Service scope in every context; app deletion requires a saved retirement review. Their direct CLI mutation paths are removed. Site rollback and preview deletion also require saved reviews in every context.
...
The generic `inventory plan` and `inventory apply` CLI now choose infrastructure preflight from selected observation requirements and reviewed operations, rather than every accepted declaration in the context. Planning opens packaged Kubernetes sources only for selected members and requires Helm native bytes only for selected releases. An isolated CLI fixture compiled, planned, and applied a new empty application scope beside unchanged Pulumi and Kubernetes scopes while fake `npm`, `pulumi`, `gcloud`, and `kubectl` commands remained untouched; both unrelated accepted revisions stayed fixed. Its recording-provider extension submits a real app command and applies its reviewed Kubernetes work beside the same unrelated owners. A disposable k3d run of the worker-only variant proved the CLI path with real Kubernetes Deployment and ConfigMap writes, while the foreign owners had no provider calls or revision changes. Knative Service and complete multi-provider membership remain unproved.

A recording transaction takes that app A candidate through observation, review publication, admission, and apply with only a Kubernetes adapter registered. Its single effect is app A's resource ID; app B and the unrelated Pulumi scope retain their exact accepted revisions. The one-invocation command fixture seeds both unrelated scopes in private context history and converges a new application with the same Kubernetes-only registry; five focused command tests passed. The later isolated CLI and disposable worker-only provider probes above extend this selection proof through the public command path.
...
2026-09-26: The full CLI deletion replay reached suspension and retention, then failed when planning collection because the command built a file-backed Kubernetes adapter for unchanged accepted members instead of passing their private native bytes. After that was corrected, apply failed because private review reconstruction assumed every Kubernetes operation names a desired declaration; exact collection names a retained resource absent from desired scopes. The collection adapter now takes that resource from the verified retained history while the public review still binds the exact collection proof and private mutation digest. The local CLI/provider sequence passes with the sibling ConfigMap UID unchanged.

2026-09-26: A compiled candidate can contain an unchanged Pulumi owner even when a new empty application scope is the only selection. The generic CLI nevertheless ran infrastructure preflight and `npm ci` because it looked at all desired registrations. It also tried to load packaged native sources for an unrelated accepted Kubernetes member. Planning now uses selected observation requirements for preflight and native source loading; apply uses the reviewed operation executors. An isolated plan/apply probe with fake provider commands completed without invoking any of them.
...
2026-09-26: The server preview renderer included managed Preview stores but filtered Preview-only inline environment entries from its Service. Reviewed preview compilation also refused their Secret references despite accepting typed Secret resource inputs. Previews now render Runtime and Preview inline entries once, bind the exact accepted Secret set for both scopes, and keep Preview-only entries out of production. Build Secret references still refuse until their image-publication input is modeled.

2026-09-26: The CLI preflight selected no host for an empty application review, but `inventory apply` still decoded an unrelated accepted host activation and constructed its adapter. That adapter read a context host configuration before observing any reviewed operation, so the empty review failed on a missing host file. The planning registry similarly built provider adapters from all accepted declarations. Filtering each adapter's execution inputs by selected resources removes those unrelated setup reads while retaining complete declaration validation and the cache-key resolver needed by selected Kubernetes members.
...
2026-09-26: Route signed static webhooks through `nagarectl site` after selecting the exact accepted commit-tagged image and, for previews, the four accepted environment stores. The runner needs a named context and initialized history; the CLI checks the selected resource IDs again. Do not build or push an image inside the webhook worker. Remove the separate static/server deploy effects while retaining their pure renderers for inventory compilation. This supersedes the earlier temporary policy of refusing all initialized contexts.

2026-09-26: Restore an accepted manual backup first into a newly named PostgreSQL scratch database. The saved restore scope pins the accepted backup receipt bytes, backup and target scope revisions, exact object addresses, target StatefulSet/PVC UIDs, and a stable restore ID. The download Job rereads receipt and backup objects, compares both SHA-256 digests, and checks expiry before decompression. Its PostgreSQL container uses `createdb` without `dropdb` and stops on SQL errors, so an uncertain or partial result cannot be retried by overwriting existing scratch data. Live-target restore and Redis/ClickHouse require separate write fences and forward-recovery contracts; exact pruning needs a provider-bound conditional delete and remains separate.

2026-09-26: Let the backup upload Pod carry the receipt bytes it fetched after create-only object publication in its Kubernetes termination message. Verification must bind that succeeded Pod to the completed Job UID, parse the receipt against the accepted native Job's exact object/receipt addresses and static metadata digest, and include its checksum and receipt digest in the journal proof. This needs cluster read access but no workstation object-store credentials or unreviewed verification Pod. Kubernetes [truncates a container termination message above 4096 bytes](https://kubernetes.io/docs/reference/kubernetes-api/core/pod-v1/), so backup compilation refuses a receipt that would exceed the bound. An absent or truncated Pod message leaves the operation unresolved. Restore and pruning must still fetch and hash the current backup object; a Pod message proves only the Job's earlier readback.

2026-09-26: Keep scheduled backup uploads in accepted database scopes, but remove inline keep-last-N deletion from their CronJob. An accepted CronJob can run repeatedly after the review; a broad object-store prefix deletion inside it has no per-object identity or lifecycle proof. Retain the old renderer only for unadmitted legacy commands and hide the low-level native database binders from the supported inventory API. Backups accumulate until M3 supplies reviewed pruning, which is preferable to deleting unclaimed recovery data.

2026-09-26: Treat inline Preview-scoped server environment as part of the preview Service render. Require exact accepted Kubernetes Secret dependencies for Preview-scoped references, using the existing site Secret input and namespace checks; production binds only Runtime references. Continue rejecting any Build-scoped Secret reference because the reviewed site scope does not prove how it entered the accepted image publication.

2026-09-26: Select provider adapter bindings from observation requirements during planning and from immutable reviewed operation resources during apply/resume. Keep global composition and registration validation, then pass only the selected provider declarations to Pulumi, host, artifact, broker, CDN, Kubernetes, and Helm adapters. Keep the complete cache-key resolver available while selected Kubernetes native bytes may refer to an unchanged accepted cache output. An idle provider must not require local configuration or credentials merely because its accepted scope is present.

2026-09-26: Match the automatically discovered GHC package-environment file to the `ghc` version that will run a typed application config. A different compiler's environment can exist beside it after an upgrade and is not usable as an implicit fallback; if no matching file exists, ask Cabal for the current environment. Explicit `--ghc-env` and `NAGARE_GHC_ENVIRONMENT` overrides remain authoritative.

2026-09-26: Choose generic CLI infrastructure preflight from selected resources at plan time and immutable reviewed operation executors at apply time. Run planning preflight before initializing or seeding inventory history, using read-only current history when present and selected replacement declarations for a new store. Composition still checks every accepted claim, while unrelated Pulumi, host, artifact, Helm, and Kubernetes declarations do not authorize or require provider setup for a different scope. Keep the more extensive application deploy isolation proof open because the current CLI probe selects an empty scope.

2026-09-26: Build the CLI collection adapter from current accepted native members plus the exact retained incarnation loaded through the verified collection proof. Desired-scope reconstruction deliberately skips collection operations because their resource has left desired intent; execution loads its immutable historical native bytes. The saved review still binds the collection operation and private native mutation digest, and admission checks the retained owner, revision, and physical identity.
...
2026-09-26: `nagared` now checks initialized history, resolves a signed event's accepted OCI publication and preview environment stores, then invokes reviewed `nagarectl site deploy` or `site preview deploy`. The old static and server build/push/apply functions were removed; pure renderers remain. The build, 845 CLI tests, webhook guard regression, Haskell style, strict user docs, and diff check passed. A successful signed webhook against a disposable native provider and in-cluster context/store setup remain M4 acceptance gaps.

2026-09-26: `app stop` and `app restart` now always select and update one accepted application or standalone Service scope through the inventory journal. `app delete` requires `--save-plan` before any source or provider access and submits only accepted-scope retirement; its direct Service/domain/history deletion branch and unused config lookup were removed. The unused direct mutation exports in `Nagare.App` and their implementation-only tests were removed. The executable build, 844 CLI tests, Haskell style, strict user docs, and both isolated entrypoint regressions passed. This advances M3/M4 operational command coverage. Live Knative stop/restart proof, collection of retained application members, and migration of `scripts/live-smoke.sh` to reviewed image/data operations remain open.

2026-09-26: `site rollback` and `site preview delete` now require saved reviews in every context. The rollback selects accepted private release history and an accepted image publication; preview deletion submits retirement before separate conditional collection. Both old direct CLI mutation branches were removed. The isolated entrypoint check proves both commands refuse without `--save-plan` before loading site source or creating inventory history. M3 and M4 remain open for other operational commands and reviewed webhook submission.

2026-09-26: Production `site deploy` and static/server `site preview deploy` now always use reviewed scope compilation and require an accepted image, explicit tag, and `--skip-build`. Their `--dry-run` routes print canonical public scopes without publication or mutation; saved and one-invocation live routes retain their existing journal and adoption behavior. The old direct static/server production and static-preview CLI branches and their direct CDN step were removed. This advances M4 deployment command coverage; `nagared` still has a separately guarded direct webhook path, and site rollback/preview deletion plus other operational commands still have legacy behavior before inventory initialization.

2026-09-26: Standalone `deploy` and `worker deploy` now enter their reviewed scope compilers for dry-run, saved review, and live execution in every context. Both require an explicit tag and accepted OCI image publication; the Service's direct build/push/Kubernetes function and the worker's entire direct deploy module were removed. The isolated CLI entrypoint check covers all three image-free application, Service, and worker commands before source loading or inventory creation. Site production and preview still have direct branches because their reviewed path does not yet supply dry-run output; M4 remains open.

2026-09-26: `app deploy` now enters the reviewed application compiler for dry-run, saved review, and live execution in every context. Live deploy requires an explicit tag and accepted `--image-resource` from `app image-plan`; the old image-free build/push and direct Kubernetes rollout branch and its live phase executor were removed. The pure per-member renderers remain because the inventory compiler consumes them. An isolated CLI entrypoint check proves an image-free deploy refuses before opening a source file or creating inventory history. This advances M4 for aggregate applications; image build preparation remains a separate publication step, and standalone Service, worker, site, and other direct command cutovers remain open.

2026-09-26: Added `db prune-backup NAME BACKUP_ID --save-plan DIR` for one expired, accepted manual backup. Planning reads the completed backup Job's UID-bound Pod receipt, refuses accepted restore/dependency users, and pins the backup scope revision, Job UID, object and receipt hashes in an independent `PruneData` scope. Apply reloads the accepted Job's private native bytes and checks its physical UID again, including when only the Job completion operation remains. The prune Job rereads both exact provider versions, checks their hashes and the expiry epoch, then deletes GCS objects with `--if-generation-match` or MinIO objects by `--version-id`; local mode checks that deleting a current version did not expose an older live version. It has no automatic retry. Reviewed local backups enable bucket versioning before upload, so old unversioned local objects refuse pruning. This choice avoids MinIO's [reported ignored conditional delete headers on the pinned release](https://github.com/minio/minio/issues/21677) and follows [Cloud Storage generation preconditions](https://docs.cloud.google.com/sdk/gcloud/reference/storage/rm) and [S3 version-specific deletion](https://docs.aws.amazon.com/AmazonS3/latest/userguide/DeletingObjectVersions.html). All 847 CLI tests, the executable build, all 459 DSL tests, Haskell style, strict user-doc validation, and diff check passed after the final source guard and provider replay changes. Live object-store provider proof and scheduled pruning remain open; M3 is still in progress.

2026-09-26: Reviewed manual backup Jobs now use `manual-databases/<namespace>/<name>/<id>.<ext>` outside the legacy schedule pruning prefix. This separates manual IDs from scheduled backups and same-named databases in different namespaces. An already accepted manual ID retains its earlier immutable Job/object address and cannot silently replan under the new layout. New Jobs render a second create-only object at `<backup-object>.receipt.json` only after the stored compressed backup passes SHA-256 readback. The receipt records format version, checked checksum, logical ID, exact object address, source scope generation/revision, StatefulSet/PVC identities, engine, namespace, and expiry; its own bytes are read back before the Job completes. The saved scope and Job annotation name the exact receipt address; the scope also pins the canonical metadata digest. `parseManualBackupReceipt` rejects a wrong receipt address, malformed checksum, or changed static metadata before returning the checksum. A fake GCS transport replay proves the stored JSON checksum and duplicate-key refusal, and all 843 CLI tests pass. The journal still records Job completion rather than fetching the object receipt, and neither pruning nor restore consumes it yet. A backup object left by a failed receipt write is deliberately not retried over the occupied key; it needs explicit recovery. Live provider proof remains open after the pinned local MinIO image returned Quay HTTP 401 in the preceding attempt.

2026-09-26: Manual backup completion now consumes the receipt read back by the upload Pod. The shell copies those exact bytes into the Pod termination message after checking both the backup and receipt hashes. The Kubernetes adapter selects a succeeded upload Pod owned by the observed Job UID, validates the receipt's address, checksum, and static metadata against the bound review, and incorporates the receipt digest and backup checksum into its completion proof. Missing, foreign, or changed Pod evidence leaves completion unresolved, including during recovery. The 843-test CLI suite and executable build pass, including fake object-store readback and adapter rejection cases. This is an at-completion receipt; fresh object-store receipt/data verification for restore and pruning and live in-pod proof remain open.

2026-09-26: Added `db restore NAME BACKUP_ID --restore-id ID --save-plan DIR` for an accepted PostgreSQL database and reviewed manual backup. Planning requires a completed accepted backup Job with an exact-UID Pod receipt, checks backup expiry, and saves an independent restore Job scope with the target revision and StatefulSet/PVC UIDs. Apply rechecks target native evidence and UIDs; the init container rereads and hashes both current object-store objects and checks expiry, then the PostgreSQL container creates a new scratch database without dropping one and stops on SQL errors. An uncertain partial result needs explicit forward recovery. The full CLI suite and executable build passed; after adding the 4096-byte Pod-message guard, focused manual tests passed. Fake object-store corruption and expiry cases pass. Live provider proof, exact pruning, live-target and other-engine restore remain open, so M3 remains in progress.

2026-09-26: Reviewed fixed-key manual backups now use a provider-side create-only upload: GCS `gcloud storage cp --if-generation-match=0`, MinIO `aws s3api put-object --if-none-match '*'` below 4 GiB and conditional `complete-multipart-upload` above it. These follow the [Cloud Storage generation-zero precondition](https://docs.cloud.google.com/storage/docs/request-preconditions), [S3 conditional writes](https://docs.aws.amazon.com/AmazonS3/latest/userguide/conditional-writes.html), [S3 multipart limits](https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html), and the [pinned MinIO release's multipart completion handler](https://github.com/minio/minio/blob/RELEASE.2025-09-07T16-13-09Z/cmd/object-multipart-handlers.go), which checks `If-None-Match` at completion. An occupied key fails the Job, including on retry after an uncertain upload, so prior backup bytes remain untouched. Scheduled reviewed CronJob bytes and legacy direct upload commands are unchanged. Fake backends replay single and multipart create, readback, and refusal to overwrite; all 841 CLI tests passed. A disposable `k3d-nagare-inventory-ep148` attempt applied the pinned local MinIO manifest, but Quay returned HTTP 401 for its MinIO image digest before the pod could start; the cluster was stopped. Live in-pod provider proof remains open.

2026-09-26: Added the reviewed manual database backup compiler and `db backup --backup-id --save-plan` route. Its own stable Job scope and `SnapshotData` operation pin the source revision and StatefulSet/PVC UIDs, exact GCS/MinIO object address, explicit retain/UTC expiry, and SHA-256 readback. The Kubernetes adapter reloads accepted source native evidence and refuses stale UID or digest before a Job write. The 840-test CLI suite passed; a disposable local k3d server accepted the Job manifest in a server-side dry run, then the cluster was stopped. A live Job/object-store replay and durable per-object receipt remain open. M3 is still in progress.

2026-09-26: Reviewed database backup schedules now compress to a temporary file, upload, read back the exact object, and compare SHA-256 before Job completion. The old unverified inventory renderer is retained only for matching accepted native bytes during a focused schedule upgrade; legacy direct bytes stay unchanged. A fake GCS/MinIO shell replay confirms successful readback and failure on changed stored bytes. A disposable local k3d replay also updated a legacy CronJob in place and read the new verification script from the live object; the cluster was stopped afterward. The full 839-test CLI suite, executable build, AST style script, and strict user-documentation validation passed. Actual in-pod object-store verification remains open, as do logical backup receipts, expiry, pruning, and restore.

2026-09-26: Added a focused saved review for accepted legacy database backup CronJobs. It checks the accepted StatefulSet and CronJob private bytes, requires an exact match to the known legacy renderer for the selected backend, and changes only the CronJob native digest and bytes. A disposable local k3d provider test created the old CronJob, applied the update, verified the same physical UID, and read a live upload script without pruning; the cluster was stopped afterward. The full 837-test CLI suite, executable build, AST style script, strict user-documentation validation, and diff check passed. The repository-wide Fourmolu check fails on existing differences across unchanged files. Already started Jobs can still finish the old pruning script after the update. Full CLI saved-review/apply proof and the remaining backup, prune, and restore operations remain M3 work.

2026-09-26: Removed legacy inline object-store pruning from reviewed database backup CronJobs and limited the exported native compiler to the backend-bound path. The full 835-test CLI suite checks that accepted native bytes contain no deletion command and the legacy renderer still retains its old behavior. Reviewed backup, prune, and restore operations remain M3 work.

2026-09-26: Proved real CLI reviewed application planning and apply on local disposable k3d with a preloaded worker image. The saved review created one available Deployment and its release-history ConfigMap; accepted unrelated Pulumi, host, artifact, and Kubernetes members kept their revisions, and no unrelated provider executable ran. A Knative Service variant could not run on a separate local cluster because its Serving webhook had no ready endpoint; full M2 membership and M4 direct-path removal remain open.

2026-09-26: Extended the local scope-isolation CLI fixture through reviewed app apply with a recording Kubernetes provider. The saved review created exactly its Service and release-history ConfigMap; unrelated accepted owners and provider executables stayed untouched. M4 still needs real native provider evidence, and M2 needs its combined provider proof.

2026-09-26: Selected generic inventory CLI preflight and packaged native sources by actual observation and operation membership; proved a Pulumi/Kubernetes foreign-owner no-op plan/apply without provider processes. M4 application-command isolation and M2 combined provider proof remain open.

2026-09-26: Proved the three reviewed task-deletion stages through the public CLI on a disposable Kubernetes context; fixed collection planning and apply reconstruction to use accepted and retained native evidence. M3 remains open for other operational/data actions.
