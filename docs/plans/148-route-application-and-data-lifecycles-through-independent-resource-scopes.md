...
- [x] (2026-09-24) M1 partial: Scheduled task CronJobs compile from the same resolved preview bytes and carry optional stable logical keys. `app deploy --save-plan` includes application and Service-local schedules only when every resolved task image matches the accepted application image; one-off Job execution remains outside this declaration step.
...
- [x] (2026-09-24) M1 partial: A standalone web Service compiles under its own scope through the same native service/PVC/domain binder as an application Service. `deploy --save-plan` binds it and its scheduled CronJobs to an accepted OCI image and Namespace, with explicit retained-volume and Secret dependencies; database, broker, access, and CDN inputs refuse this route.
...
- [x] (2026-09-24) M1/M2 partial: Reviewed application planning resolves supplied-TLS and runtime-environment Secret resource IDs from accepted scope history. The compiler then checks exact dependency name, namespace, cluster, and reference-set equality; application-owned database credentials remain internal bindings.
- [x] (2026-09-24) M1/M3 partial: Reviewed application planning accepts explicit recovery bindings for every retained Service and worker PVC. Worker bindings resolve to the volume's stable ResourceId, and missing, duplicate, unknown, or throwaway-volume decisions refuse before review.
...
- [x] (2026-09-24) M2/M4 partial: `app deploy --save-plan --request-namespace` submits the typed closed namespace contribution through the accepted platform foundation cluster. The composer enforces the platform owner's explicit grant and produces native Namespace bytes without advancing the foundation scope revision; the default path still requires an accepted Namespace.
- [x] (2026-09-24) M4 partial: Direct application deploy checks companion PVCs, DomainMappings, database credentials, Services, ConfigMaps, and backup CronJobs against accepted and retained native addresses. Direct database and broker create check the resolved Config.hs identity and every companion object before provider mutation, closing positional-name bypasses; focused tests and the operator build pass.
- [x] (2026-09-24) M1/M2 partial: Reviewed application planning binds topic-free application, Service, and worker broker references to an accepted standalone broker Service and StatefulSet in the same cluster/namespace. Kafka connection env derives from accepted identity, and only consuming workload declarations/private native members depend on the Service. Unaccepted, duplicate, conflicting, and topic-bearing bindings refuse; logical topic operations remain open.
- [x] (2026-09-24) M2 partial: `app image-plan` binds an absolute Docker archive's SHA-256 and OCI manifest digest to a scoped artifact publication review. The generic OCI transport checks the archive, manifest, selected registry, and reviewed destination before copying; a changed archive refuses without invoking copy. Accepted image identity then feeds `app deploy --image-resource`. Application build inputs and automatic build-to-review remain open.
- [x] (2026-09-24) M2 partial: Exact Runtime, Build, and app-wide Preview `env sync --save-plan` and `secret sync --version --save-plan` compile separate ConfigMap and Secret scopes and save shared inventory reviews. Private native bytes match the existing scope-specific stores; later app scope replacements leave these channels' revisions and values alone. Merge intent, per-preview inputs, and reviewed preview deployment remain open.
