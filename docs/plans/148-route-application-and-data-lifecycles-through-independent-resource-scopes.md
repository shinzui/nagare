...
Newly compiled reviewed database scopes now emit their scheduled backup CronJob without the legacy inline keep-last-N object-store deletion. The supported inventory compiler is the only exported native database binder; its saved private CronJob bytes upload a backup but contain no pruning command. Existing accepted CronJobs keep their earlier native bytes until a fresh database review replaces them. Legacy direct database commands retain their earlier renderer for unadmitted contexts. This prevents new reviewed schedules from deleting backup objects without lifecycle proof, while reviewed backup identity, object verification, explicit pruning, and restore operations remain M3 work.
