...
Newly compiled reviewed database scopes now emit their scheduled backup CronJob without the legacy inline keep-last-N object-store deletion. The supported inventory compiler is the only exported native database binder; its saved private CronJob bytes upload, read back, and SHA-256-check the exact object before the Job succeeds, with no pruning command. `db disable-backup-prune --save-plan` can replace an earlier accepted CronJob through a match against the known older renderers while preserving its sibling declarations and native evidence. Existing CronJobs retain their earlier script until that review is applied. Legacy direct database commands retain their earlier renderer for unadmitted contexts. The scheduled readback is an execution check without a durable per-object receipt or expiry policy; exact pruning and restore operations remain M3 work.

An accepted database can now save a manual backup review with `db backup NAME --backup-id ID --save-plan DIR`. The fixed Job has its own stable scope and namespace-isolated object key under `manual-databases/<namespace>/<name>/` outside the legacy schedule pruning prefix, an explicit retain or UTC expiry choice, the accepted source scope revision, StatefulSet/PVC UIDs, and the exact stored-byte SHA-256 check. A `SnapshotData` operation journals completion after the Job. Execution reloads accepted source native bytes and checks both physical UIDs before submit and verification; a changed accepted backup intent under the same ID refuses. The Job now renders create-only uploads for both the backup and a `.receipt.json` object with the checked SHA-256, source proof, object URL, and expiry; it reads both objects back before success. A pure receipt parser checks a fetched receipt's address, checksum shape, and accepted metadata digest. The backup Job remains inventory intent. Provider proof, inventory consumption of the object receipt, exact pruning, and reviewed restore are still open. A partial upload can leave an unreceipted backup that requires explicit recovery. Source checks are not atomic with Job execution. Direct manual backup remains guarded after inventory initialization.
