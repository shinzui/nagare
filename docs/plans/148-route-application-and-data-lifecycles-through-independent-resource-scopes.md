...
- [x] (2026-09-24) M1 partial: A standalone web Service compiles under its own scope through the same native service/PVC/domain binder as an application Service. `deploy --save-plan` now binds it to an accepted OCI image and Namespace, with explicit retained-volume and Secret dependencies; database, broker, task, access, and CDN inputs refuse this route.
