...
- [x] (2026-09-24) M2 partial: Exact Runtime and Build `env sync --save-plan` and `secret sync --version --save-plan` compile separate ConfigMap and Secret scopes and save shared inventory reviews. Private native bytes match the existing scope-specific stores; later app scope replacements leave these channels' revisions and values alone. Preview and merge intent channels remain open.
