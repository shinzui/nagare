...
An accepted database can now save a manual backup review with `db backup NAME --backup-id ID --save-plan DIR`. The fixed Job has its own stable scope and object key, an explicit retain or UTC expiry choice, the accepted source scope revision, StatefulSet/PVC UIDs, and the exact stored-byte SHA-256 check. A `SnapshotData` operation journals completion after the Job. Execution reloads accepted source native bytes and checks both physical UIDs before submit and verification; a changed accepted backup intent under the same ID refuses. Fixed-key uploads now use provider-side create-only preconditions rather than replacing an occupied object. The backup Job is retained as inventory intent, while an actual per-object checksum receipt, exact pruning, and reviewed restore are still open. Source checks are not atomic with Job execution. Direct manual backup remains guarded after inventory initialization.
