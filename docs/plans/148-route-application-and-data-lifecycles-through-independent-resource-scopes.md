...
Newly compiled reviewed database scopes now emit their scheduled backup CronJob without the legacy inline keep-last-N object-store deletion. The supported inventory compiler is the only exported native database binder; its saved private CronJob bytes upload a backup but contain no pruning command. `db disable-backup-prune --save-plan` can replace an earlier accepted CronJob through an exact legacy-renderer match while preserving its sibling declarations and native evidence. Existing CronJobs retain their earlier script until that review is applied. Legacy direct database commands retain their earlier renderer for unadmitted contexts. This prevents new reviewed schedules from deleting backup objects without lifecycle proof, while reviewed backup identity, object verification, explicit pruning, and restore operations remain M3 work.
