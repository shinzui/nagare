...
- [x] (2026-09-24) M2 decision replay boundary: Validated lifecycle decisions now carry their exact composed candidate. The planner revalidates them against current history and observations; mixing decisions from different candidates or deciding twice for one resource refuses. A focused adoption fixture and the 751-test CLI suite pass. Provider adoption breadth and migration remain open.
...
An opaque lifecycle decision could be passed to `planChanges` with a later candidate or observation. Revalidating its proposal detects observation changes; carrying the original composition candidate also detects declaration changes that preserve the observation. Disjoint decisions can now be combined only when both were validated for the same candidate.
