...
- [ ] M1b remaining: Prove safe field ownership transition and update behavior across supported kinds, plus live stale UID/resourceVersion and foreign-object refusal through the registered command path. The current non-forced update request can legitimately conflict with its own create ownership.
...
2026-09-22: The first production kubectl transport is registered for file-backed declarations. It uses create-only POST semantics for absence and non-forced server-side apply with UID/resourceVersion for updates, with an explicit kubectl context and a context guard callback. A disposable ConfigMap create completed and verified. Update may refuse on the create manager's own Update ownership; this is a safe incomplete behavior, not M1b completion. Generated namespace declarations lack a packaged source and still require the M3 composer. Private review reconstruction avoids rereading mutable manifest paths at apply/resume.
