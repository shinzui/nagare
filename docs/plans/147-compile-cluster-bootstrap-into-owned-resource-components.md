...
2026-09-22: A Service selector update passed, but changing an unnamed Service port from 8080 to 8081 via server-side apply failed validation: the API server temporarily treated the old and new merge-key entries as two unnamed ports. A per-kind Service port transition needs a guarded patch or a named-port requirement; the current transport returns an ambiguous result and requires observation. This is why a ConfigMap-only success cannot be generalized to every Kubernetes kind.
