...
| Resource inventory history | Private local directory under `${XDG_STATE_HOME:-$HOME/.local/state}/nagare/<context>/inventory`, or opt-in GCS prefix in the context state bucket | 🟡 (back up local history with `nagarectl inventory export --out DIRECTORY`; GCS history is shared between workstations, but live migration rehearsal is pending — see [Shared inventory history](contexts.md#shared-inventory-history-cloud-contexts)) |
