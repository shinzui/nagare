...
import Nagare.Resource.Policy (DataPolicy (Stateless), LifecyclePolicy (Protect), Sensitivity (Private))
...
pinnedObservabilityInputs cluster root kubeVersion =
  [ pinned
      "vmks"
      "monitoring"
      "victoria-metrics-k8s-stack-0.81.0.tgz"
      "34e2dfafb05dfdcf85aac05c8e61fa5e2f2dd33672422d4e09c066e09b664c13"
      "victoria-metrics/values.yaml"
      "5d71af726d8edf01571ee98b182f6ba4fd8eba7088b35d16e71b6460f82f3e57"
  , pinned
      "victoria-logs"
      "logging"
      "victoria-logs-single-0.13.5.tgz"
      "b08975ef8b9f47e707df41e81cc55621baee9073a49de75a480bb98345502f61"
      "victoria-logs/values.yaml"
      "a4d2847c50a84b7d50cb4e7df85bec50b2edc716e07716a6c30a6f0830b97a59"
  , pinned
      "victoria-logs-collector"
      "logging"
      "victoria-logs-collector-0.3.4.tgz"
      "2196ce5622722268fc1efb430393e9993f42905a85a4451d8e3465343ff0dc6c"
      "victoria-logs/collector-values.yaml"
      "5c5d7e7fe34be9a73e77674ca351c79906ce2b031b2e0641c752a2fdd967ec6e"
  , pinned
      "victoria-traces"
      "tracing"
      "victoria-traces-single-0.1.6.tgz"
      "3fd7b2e2625590d4ffcbfe358a9f74fc9fc860c9d9485974ac162d4cd196fc74"
      "victoria-traces/values.yaml"
      "73a59e064b60912d44c14bdb59aa35e5c17aecb8ceb4a7ceeb3749d9d5d32dda"
  , pinned
      "otel-collector"
      "tracing"
      "opentelemetry-collector-0.158.0.tgz"
      "b0c8f6b8f3eff6bc14af492a8f30df3353a685eae28aca3855559c6d20d76d39"
      "opentelemetry-collector/values.yaml"
      "ebd92c3ae2b0f6e224b04bfa3be201f1c799e59a61f0a52856c5a4492c1054d5"
  ]
  where
  ...
  where
    one input = do
      ...
      pure $ do
        ...
        let namespaceContribution =
              RegisterNamespace
                foundationOwner
                (packagedCluster input)
                (packagedNamespace input)
                (knownKey "namespace")
            namespaceId =
              mintResourceId
                foundationOwner
                (knownKey (nameText (packagedNamespace input)))
                (knownName "namespace")
            orderedRelease =
              release
                { dependencies =
                    map (OrderedAfter . (^. #identity) . fst) crds
                      <> [OrderedAfter namespaceId]
                      <> release ^. #dependencies
                }
            bundle =
              ResourceBundle
                (map (Managed . fst) crds <> [Managed orderedRelease])
                []
                []
        ...
        pure
          ( scope
          , Map.fromList
              ( [(resource ^. #identity, (resource, bytes)) | (resource, bytes) <- crds]
                  <> [(orderedRelease ^. #identity, (orderedRelease, native))]
              )
          )
    invalid input message =
      inventoryError "invalid-observability-release" message
        & #scopes
        .~ [packagedOwner input]
        & #resources
        .~ [packagedReleaseId input]
    single err = err :| []
  ...
  case (chart, values) of
    ...
    (Right chartBytes, Right valuesBytes)
      ...
      | otherwise -> do
          attempted <-
            try
              ( withSystemTempDirectory "nagare-helm-capture" $ \temporary -> do
                  ...
                  (versionCode, versionOutput, versionError) <-
                  ...
                  let clean = filter (\(key, _) -> key `notElem` ["HELM_PLUGINS", "NAGARE_HELM_CAPTURE_PATH"]) environment
                      ...
                      base =
                        ...
                        , chartPath
                        , "--namespace"
                        , T.unpack (nameText (packagedNamespace input))
                        , "--values"
                        , valuesPath
                        , "--kube-version"
                        , T.unpack (packagedKubeVersion input)
                        , "--skip-crds"
                        , "--post-renderer"
                        , "nagare-capture-manifests"
                        ]
                  ...
                  if versionCode /= ExitSuccess
                    then pure (Left ("Helm version probe refused: " <> T.pack versionError))
                      ...
                      if T.strip (T.pack versionOutput) /= packagedHelmVersion input
                        then pure (Left "Helm version differs from the pinned render engine")
                        else do
                          (renderCode, _, renderError) <-
                            readCreateProcessWithExitCode
...
compileRenderedRelease input = do
  ...
  unless
    (null [address | address <- crdAddresses, address `elem` members])
  ...
  contract <-
    first invalid $
      canonicalValue $
        object
          [ "chartPath" .= portablePath (releaseChartPath input)
          , "chartDigest" .= digestText (contentDigest (releaseChartBytes input))
          , "valuesPath" .= portablePath (releaseValuesPath input)
  ...
  where
    portablePath path =
      let (_, suffix) = T.breakOn "/cluster/observability/" (T.pack path)
       in if T.null suffix
            then T.pack path
            else "payload:" <> T.drop (T.length ("/cluster/observability/" :: Text)) suffix
    source = SourceLocation (T.pack (releaseChartPath input)) (nameText (releaseName input))
    invalid message =
      inventoryError "invalid-helm-release" message
        & #scopes
        .~ [releaseOwner input]
        & #resources
        .~ [releaseId input]
...
compileCrds input = do
  ...
  where
    ...
    invalid message =
      inventoryError "invalid-helm-crd" message
        & #scopes
        .~ [releaseOwner input]
        & #sources
        .~ [source]
    one (location, value) = do
      ...
      unless
        (KM.lookup "kind" root == Just (String "CustomResourceDefinition"))
