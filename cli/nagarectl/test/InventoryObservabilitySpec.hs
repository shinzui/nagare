...
inventoryObservabilityTests =
  testGroup
    ...
    , testCase "packaged Helm contract survives a payload root change" $ do
        let packaged root =
              fixture
                { releaseChartPath = root <> "/cluster/observability/vendor/metrics-1.0.tgz"
                , releaseValuesPath = root <> "/cluster/observability/metrics/values.yaml"
                }
            firstRelease = ok (compileRenderedRelease (packaged "/tmp/payload-a"))
            secondRelease = ok (compileRenderedRelease (packaged "/tmp/payload-b"))
        snd firstRelease @?= snd secondRelease
        (fst firstRelease ^. #spec) @?= (fst secondRelease ^. #spec)
        ...
        let foundationOwner = ok (mkScopeId Platform "foundation")
            ...
            foundation =
              FoundationInput
                ...
                (map packagedOwner inputs)
        (secretScope, secretNative, secretIds) <-
          compileObservabilitySecrets foundation secretObjects
            >>= either (assertFailure . show) pure
        assertBool
          "observability Secret retained stringData or plaintext"
          ( all
              ( \(_, bytes) ->
                  not ("stringData" `BC.isInfixOf` bytes)
                    && not ("fixture-canary" `BC.isInfixOf` bytes)
              )
              (Map.elems secretNative)
          )
        assertBool
          "observability Secret was not normalized to data"
          (any (BC.isInfixOf "Zml4dHVyZS1jYW5hcnk=" . snd) (Map.elems secretNative))
        let orderedInputs = case inputs of
              firstRelease : rest ->
                firstRelease
                  { packagedDependencies = map OrderedAfter secretIds <> packagedDependencies firstRelease
                  }
                  : rest
              [] -> []
    ...
    , testCase "observability Secret input follows pinned Alertmanager policy" $ do
        let foundation =
              FoundationInput
                (ok (mkScopeId Platform "foundation"))
                cluster
                "../../cluster/bootstrap/job-runs/resourcequota.yaml"
                []
        ...
        assertBool
          "missing Grafana admin Secret was accepted"
          (case absent of Left _ -> True; Right _ -> False)
        let overlapping =
              [
                ( SourceLocation "fixture:overlap" "Secret"
                , object
        ...
        assertBool
          "overlapping Secret data and stringData was accepted"
    ...
    , testCase "encrypted observability input is required and decryption errors stay private" $
        withSystemTempDirectory "observability-secrets" $ \directory -> do
          ...
          loaded <-
            loadObservabilitySecretObjectsFromDirectory executable directory False
              >>= either (assertFailure . T.unpack) pure
          length loaded @?= 1
          required <- loadObservabilitySecretObjectsFromDirectory executable directory True
          assertBool
            "missing enabled Alertmanager configuration was accepted"
    ...
    , testCase "cloud bootstrap composes auth, cache publication, and observability" $
        withSystemTempDirectory "bootstrap-complete" $ \root -> do
          let source = "../../cluster/bootstrap/nix-cache"
              ...
              foundation =
                FoundationInput
                  ...
                  cluster
                  "../../cluster/bootstrap/job-runs/resourcequota.yaml"
                  (map packagedOwner observabilityInputs)
              images =
                Map.fromList
                  [ ( service
          ...
          (imageScope, cacheScope, cacheNative) <-
          ...
          (controllerScope, controllerImage, publication) <-
            either
              (assertFailure . show)
              pure
          ...
          (base, baseNative) <-
            compileBootstrapWithAuth
              snapshot
              (BootstrapInput foundation Nothing upstream [controllerScope])
              auth
              databases
              >>= either (assertFailure . show) pure
          let orderedInputs = case observabilityInputs of
                firstRelease : rest ->
                  firstRelease
                    ...
                    : rest
                [] -> []
          (observability, obsNative) <-
            compilePinnedObservability owner orderedInputs
              >>= either (assertFailure . show) pure
          ...
          (extras, extraNative) <-
            compileObservabilityExtras
              ...
              >>= either (assertFailure . show) pure
          let changes =
                candidateChanges base
                  <> ( ReplaceScope imageScope
                         :| ( ReplaceScope cacheScope
                                : map ReplaceScope observability
          ...
          let marker =
                object
                  [ "apiVersion" .= ("v1" :: T.Text)
                  , "kind" .= ("ConfigMap" :: T.Text)
                  , "metadata"
                      .= object
                        [ "name" .= ("nagare-platform-version" :: T.Text)
              ...
              adapter executor =
                Adapter
                  ...
                  , adapterExecute = \operation _ -> do
                      ...
                      if plannedOperationId operation == plannedOperationId migration
                        then do
                          initialAttempt <- atomicModifyIORef' firstAttempt (\value -> (False, value))
                          pure
                            ( if initialAttempt
                                then AdapterEffectAmbiguous "lost migration acknowledgement"
          ...
          let oldMarker =
                [ value
                | (_, scoped) <- Map.elems (historyAccepted convergedHistory)
                , bundle <- scopeBundles scoped
                , value <- declarations bundle
                , declarationId value `elem` stampIds
                ]
              newMarker =
                [ value
                | value <- inventoryDeclarations (candidateInventory replay)
                , declarationId value `elem` stampIds
                ]
          case (oldMarker, newMarker) of
            ([Managed old], [Managed new]) -> do
              ...
              assertBool
                ( "marker dependency mismatch: old-only "
                    <> show (Set.toList (oldDeps `Set.difference` newDeps))
                    <> ", new-only "
                    <> show (Set.toList (newDeps `Set.difference` oldDeps))
                )
                (oldDeps == newDeps)
              assertBool
                ( "marker dependency order differs at "
                    <> show
          ...
          let checks =
                [ operation
                | operation <- proposalOperations replayProposal
          ...
          assertBool
            "accepted bootstrap scheduled unchanged resources for update"
          ...
          case replayMarker of
            [markerCheck] -> do
              ...
              assertBool
                "bootstrap marker skipped an accepted native check"
                ( all
                    (\check -> plannedOperationId check `elem` plannedDependencies markerCheck)
    ...
    , testCase "reviewed Helm adapter refuses a changed release revision" $ do
        let (release, native) = ok (compileRenderedRelease fixture)
            operation =
              PlannedOperation
                (ok (mkOperationId "op-helm-create"))
                CreateResource
                HelmExecutor
                (releaseId fixture :| [])
                ...
                HelmAdapterOps
                  ...
                  , helmMutateConditional = \mutation -> do
                      writeIORef
                        current
                        ( HelmPresent
                            (ok (mkPhysicalIdentity "helm-1"))
                            "1"
                            (helmMutationResource mutation)
                            (helmMutationContractDigest mutation)
                        )
    ...
    , testCase "Helm observation reports release drift and foreign ownership" $ do
        let (release, native) = ok (compileRenderedRelease fixture)
            ...
            verify =
              PlannedOperation
                (ok (mkOperationId "op-helm-verify"))
                VerifyResource
                HelmExecutor
                (resource :| [])
                (contentDigest native)
            ...
            adapterFor current =
              mkHelmAdapter
                (Map.singleton resource (release, native))
                HelmAdapterOps
                  { helmObserve = \_ -> readIORef current
              ...
              ( do
                  ...
                  let runtime =
                        HelmRuntimeConfig
                          ...
                          (ok (mkContextId "helm-test"))
                          "../../cluster/observability/helm-review"
                          (Map.singleton (releaseId fixture) release)
                          (pure (Right ()))
                      adapter =
                        mkHelmAdapter
                          (Map.singleton (releaseId fixture) (release, native))
                          (helmRuntimeOps runtime)
                      operation =
                        PlannedOperation
                          (ok (mkOperationId "op-helm-runtime"))
                          CreateResource
                          HelmExecutor
                          (releaseId fixture :| [])
                          (contentDigest native)
                  ...
                  (readCode, actual, readError) <-
                    kubectl
                      [ "get"
                      , "configmap"
                      , "inventory-review-static-fixture"
                      , "--namespace"
                      , namespace
                      , "-o"
                      , "jsonpath={.data.token}"
  ...
  where
    ...
    secretObjects =
      [
        ( SourceLocation "fixture:grafana-admin" "Secret"
        , object
            [ "apiVersion" .= ("v1" :: T.Text)
            , "kind" .= ("Secret" :: T.Text)
            , "metadata"
